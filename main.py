...
def get_clean_data():
    df = pd.read_csv("C:/Users/parch/streamlit-cancer/data/cancer-diagnosis.csv")
...
def display_predictions(input_data, model, scaler):
    ...
    st.write("Our machine learning model analyzes tissue sample features to provide predictions. However, please note that this tool is for informational purposes only and should not replace professional medical advice. Always consult with a qualified healthcare provider for accurate diagnosis and treatment recommendations.")
...
def create_app():
    ...
    # load css
    with open("C:/Users/parch/streamlit-cancer/assets/style.css") as f:
        st.markdown('<style>{}</style>'.format(f.read()),
                    unsafe_allow_html=True)

    with st.container():
       st.title("Breast Cancer Diagnosis Assistant")
       st.write("Welcome to the Breast Cancer Diagnosis Assistant. This tool is designed to aid medical professionals in diagnosing breast cancer based on tissue sample measurements. Connect your cytology lab data and utilize our machine learning model to predict whether a breast mass is benign or malignant. You can also adjust the measurements manually using the sliders in the sidebar.")
